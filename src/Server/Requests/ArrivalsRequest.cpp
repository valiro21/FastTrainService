...
std::string ArrivalsRequest::solve (json request) {
    std::string query = "MATCH (a:Station)-[t1:DEST_TO_TRAIN]->(t:Train)-[t2:TRAIN_TO_DEST]->(b:Station) WHERE b.city='"
                        + request["city"].get<std::string>() + "' AND b.station='"
                        + request["station"].get<std::string>() + "' RETURN a, t, t1, t2";
    ...
    DatabaseManager::query(query, [&](neo4j_result_stream_t *stream){
        ...
        try {
            ...
            Logger::GetInstance().loge("ArrivalsRequest - Invalid request to database");
